...
**通过 API 登录超级管理员**（PocketBase 0.22+ admin 已迁移到 `_superusers` 集合）：

```bash
curl -X POST http://127.0.0.1:8085/api/collections/_superusers/auth-with-password \
  -H 'Content-Type: application/json' \
  -d '{"identity":"admin@geo.local","password":"admin123456"}'
```

拿到 `token` 后，在后续请求中用 `-H "Authorization: <token>"` 鉴权即可调用 `/api/collections`、`/api/collections/<name>/records` 等管理端点。

凭证由 [run.sh:46](run.sh) 在首次启动时通过 `pocketbase superuser upsert` 自动创建。
...
打开 http://localhost:8085 即可（Docker 模式下前端由 PocketBase 同源托管）。详见 [Docker 部署](README.md#docker-部署)。
...
- JSON 批量导入文章库
- 配图提示词生成与 AI 生图
- 文章审核流：draft → review → approved → archived（「已发布」状态由发布任务记录，文章本身不持有）
...
- 🌗 暗色模式
- ⌘K 全局搜索 · ⌘S 保存 · ⌘/ 打开 AI 助手（跨产品/问题/文章/任务）
...
- 提交信息遵循 [Conventional Commits](https://www.conventionalcommits.org/)：
  ...
  - `refactor:` 重构
  - `chore:` 杂项
- 代码风格：2 空格缩进，单引号字符串，末尾无分号（与 [.editorconfig](.editorconfig) 和 [CONTRIBUTING.md](CONTRIBUTING.md) 保持一致）
