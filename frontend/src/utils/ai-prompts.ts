import { callAI, aiFetch } from '@/api/ai'

// ===== 辅助函数 =====

function formatPlatforms(platforms: any[], maxCount = 20): string {
...
function formatGeoQuestions(questions: any[], maxCount = 20): string {
...
export function buildProductPrompt(product: any, platforms: any[], count: number, contentTypes: string[], questions: any[]): string {
...
export function buildGeoQuestionPrompt(product: any, count = 12): string {
  const discoveryCount = Math.floor(count * 0.3)
  const comparisonCount = Math.floor(count * 0.25)
  const howtoCount = Math.floor(count * 0.25)
  const evaluationCount = Math.max(0, count - discoveryCount - comparisonCount - howtoCount)
...
export function buildProductProfilePrompt(rawText: string, currentProduct?: any): string {
  let currentLines = ''
  if (currentProduct) {
    currentLines = `
当前表单已有信息：
...
export function buildAdaptationPrompt(article: any, platform: any, product?: any): string {
  let productLines = ''
  if (product) {
    productLines = `
产品背景：
...
export function buildContentRepurposePrompt(article: any, platforms: any[], product?: any): string {
...
export function buildPublishChecklist(article: any, platform: any): string {
...
export function buildSeoOptimizePrompt(article: any, keywords: string[]): string {
...
export function buildPlatformProfilePrompt(input: string): string {
  return `你是一个熟悉中文内容平台运营的专家，了解各平台的发布规则、内容风格和流量特点。

请根据用户输入的平台名称或公司名，整理出该平台的内容发布档案信息。

用户输入：${input}

要求：
1. 只基于公开可查的平台信息，不要编造不存在的数据
2. 如果是不知名的平台，合理推断其所属分类和一般性规则
3. 输出必须是 JSON 对象，不要 Markdown 代码块，不要额外解释

JSON 字段必须包含：
- name：平台名称
- category：平台分类，如"综合资讯"、"社区社交"、"短视频"、"技术社区"、"搜索引擎"、"问答平台"、"电商导购"、"行业垂直"
- url：平台发布入口 URL（不确定则留空）
- account_name：建议的账号名称（留空，由用户填写）
- content_style：适合的内容类型和风格描述
- recommended_words：推荐字数范围，如"800-2000字"
- title_style：标题风格建议，如"疑问式、列表式、干货型"
- tags_rule：标签使用规则，如"3-5个标签，含核心关键词"
- allows_external_links：外链限制，可选值 "allowed"（允许外链）、"limited"（有限外链）、"forbidden"（禁止外链）
- soft_article_fit：软文适配度，可选值 "high"（高适配）、"medium"（中适配）、"low"（低适配）
- frequency：建议发布频率，如"每周2-3篇"
- login_notes：登录或发布提示，如"需App发布"、"需企业认证"
- notes：其他备注信息`
}

// ===== 软文提示词生成（给用户复制到豆包/WorkBuddy 等平台用） =====
...
export function buildArticlePrompt(
  ...
  const platformDesc = platforms.length > 0
    ? platforms.map(p => `  - ${p.name}：${p.content_style || ''}；标题风格：${p.title_style || ''}；字数：${p.recommended_words || ''}`).join('\n')
    : '  - 自行选择发布平台'

  const questionLines = questions.length > 0
    ? questions.slice(0, 15).map(q => `  ${q.question}（${q.intent || '发现'}意图）`).join('\n')
    : '  - 请根据产品自行推断用户可能问 AI 的问题'
  ...
  const tone = product.tone || '真实、专业、克制'

  return `你是一位资深中文内容运营专家，擅长 GEO（生成式引擎优化）和产品内容营销。请基于以下产品信息，帮我撰写一篇高质量的${types}。
...
- **品牌语气**：${tone}
- **核心卖点**：
${product.selling_points ? '  ' + product.selling_points.split('\n').filter(Boolean).join('\n  ') : '  - 待补充'}
- **竞品 / 替代品**：${product.competitors || '无'}
- **转化目标**：${product.goal || '品牌认知 + 用户转化'}
- **禁用表达**：${product.forbidden_words || '无'}
...
1. **真实可信**：不要编造不存在的数据、奖项、用户评价、客户案例
2. **自然推荐**：广告感要弱，像真实用户/专业作者自然推荐，多讲价值少吹牛逼
3. **GEO 优化**：
   - 开头 100 字内直接回答用户可能问的问题
   - 使用清晰的标题结构（## / ###）便于 AI 解析
   - 包含具体数据、案例、对比表格
...
4. **SEO 优化**：
   ...
   - 正文使用 H2/H3 分段，每段聚焦一个要点
   - 关键信息用**加粗**或列表突出
5. **字数要求**：根据目标平台调整，短文 800-1500 字，长文 2000-4000 字
6. **结构建议**：标题 → 摘要/导语 → 正文（3-5 个分段）→ 总结/推荐 → FAQ（可选）
${noteSection}
请直接输出完整的 Markdown 格式文章，标题用 #，副标题用 ##，重点内容用**加粗**。`
}


// ===== 图片生成 =====

export async function callImageGeneration(prompt: string, aiSettings: any): Promise<any> {
...
export async function checkGeoRank(settings: any, productName: string, question: string): Promise<any> {
  const messages = [
...
export function parseGeoRank(raw: string, productName: string): any {
  const text = String(raw || '')
  let answer = text, recs: any[] = []
  const marker = '###RECOMMENDATIONS###'
  const idx = text.indexOf(marker)
  if (idx >= 0) {
    answer = text.slice(0, idx).trim()
    try { recs = extractJsonArray(text.slice(idx + marker.length)) } catch { recs = [] }
  }
  const norm = (s: string) => String(s || '').toLowerCase().replace(/[\s_,，、.。!！?？:：;；"'`""'()（）\[\]【】\-—_/\\@#~|]/g, '')
  ...
  const rank = recIdx >= 0 ? recIdx + 1 : (inAnswer ? recs.length + 1 : 0)
...
export const PLATFORM_BEST_TIME: Record<string, { weekday: string; time: string; note: string }> = {
...
export function getPlatformBestPublishTime(platformName: string): { platform: string; weekday: string; time: string; note: string } | null {
